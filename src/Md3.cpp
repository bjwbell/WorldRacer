...
#include "../include/MD3Functions.h"
...
void CModelMD3::SetFogDensity(float density){
		fogdensity=density;
}
void CModelMD3::SetFogDepth(float depth){
		fogdepth=depth;
};
///////////////////////////////// DESTROY MODEL \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
/////
    ...
    for(int i = 0; i < pModel->numOfMaterials; i++)
    {
        ...
        if(strlen(pModel->pMaterials[i].strFile) > 0)
        {
            ...
            for(int j = 0; j < strTextures.size(); j++)
            {
                ...
                if(!strcasecmp(pModel->pMaterials[i].strFile, strTextures[j].c_str()) )
                {
...
/////
///////////////////////////////// RENDER MODEL \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
void CModelMD3::FreezeAnimation(){
	freeze_animation=true;
};
void CModelMD3::UnFreezeAnimation(){
	freeze_animation=false;
};
void CModelMD3::RenderModel(t3DModel *pModel)
{
    ...
    glFogf(GL_FOG_END, 256);               // Set the end position for the detph at 50
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
				   ...
				   // SetFogCoord(fogdepth,vPoint1.y + pModel->t * (vPoint2.y - vPoint1.y)+position.y,fogdensity);
					SetFogCoord(fogdepth,vPoint1.y + pModel->t * (vPoint2.y - vPoint1.y)+position.y,fogdensity);
...
void CModelMD3::SetPos(vector3 par_position){
